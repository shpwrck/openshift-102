...
The deployed showroom (HTTP image) also serves a static **Helm chart repository** at **`/helm/`** (for example, `https://<your-openshift-route>/helm/` with `index.yaml` and a packaged `openshift-102-workshop-*.tgz`). You can add it with `helm repo add` using that **HTTPS base URL** (trailing slash optional) and `helm search` / `helm install` from a workstation; no Git clone required for the chart.
